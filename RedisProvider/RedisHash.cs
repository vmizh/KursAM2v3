...
/// <summary>
///     A RedisHash containing RedisValue keys and values.  The RedisValueHash allows for different types for key-value
///     pairs within the hash.
/// </summary>
...
public class RedisHash<TKey, TValue> : RedisObject, IAsyncEnumerable<KeyValuePair<TKey, TValue>>
{
    ...
    {
        var fields = keys.Select(k => ToRedisValue(k)).ToArray();
    ...
    public Task<TValue> Get(TKey key)
    {
        return Executor.HashGetAsync(KeyName, ToRedisValue(key))
            .ContinueWith(r => ToElement<TValue>(r.Result),
                TaskContinuationOptions.ExecuteSynchronously | TaskContinuationOptions.OnlyOnRanToCompletion);
    }
    ...
    public Task<IList<KeyValuePair<TKey, TValue>>> ToList()
    {
        return Executor.HashGetAllAsync(KeyName)
            .ContinueWith<IList<KeyValuePair<TKey, TValue>>>(
                r => r.Result.Select(p =>
                    new KeyValuePair<TKey, TValue>(ToElement<TKey>(p.Name), ToElement<TValue>(p.Value))).ToList(),
                TaskContinuationOptions.ExecuteSynchronously | TaskContinuationOptions.OnlyOnRanToCompletion);
    }

...
public class RedisList<T> : RedisObject, IAsyncEnumerable<T>
{
    ...
    public Task<T> Index(long ix)
    {
        return Executor.ListGetByIndexAsync(KeyName, ix)
            .ContinueWith(i => ToElement<T>(i.Result),
    ...
    public Task<T> RemoveFirst()
    {
        return Executor.ListLeftPopAsync(KeyName)
            .ContinueWith(r => ToElement<T>(r.Result),
                TaskContinuationOptions.ExecuteSynchronously | TaskContinuationOptions.OnlyOnRanToCompletion);
    }
    ...
    /// <summary>
    ///     Performs Redis RPOPLPUSH command. Removes last element from this list and inserts it to the head of the target
    ///     list.
    /// </summary>
    ...
    /// <param name="getKeyNamePattern"></param>
    /// <returns></returns>
    public Task<long> SortAndStore(RedisList<T> destinationList, Order order = Order.Ascending,
        SortType sortType = SortType.Numeric, long skip = 0, long take = -1, string byKeyNamePattern = null,
        string[] getKeyNamePattern = null)
    {
        var getKeys = getKeyNamePattern == null ? null : getKeyNamePattern.Select(s => (RedisValue)s).ToArray();
        return Executor.SortAndStoreAsync(destinationList.KeyName, KeyName, skip, take, order, sortType,

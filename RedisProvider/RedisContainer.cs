...
/// </summary>
public class RedisContainer(RedisConnection cn, int dbId = -1, string keyNameSpace = "", bool trackObjects = true)
    : IProxy
{
    private readonly Dictionary<string, RedisObject> _trackedObjects = new Dictionary<string, RedisObject>();
    public IDatabase Database { get; } = dbId < 0 ? cn.Connection().GetDatabase() : cn.Connection().GetDatabase(dbId);

...
/// </summary>
public class RedisValueItem(string keyName) : RedisItem<RedisValue>(keyName);
...
public class RedisItem<T>(string keyName) : RedisObject(keyName)
{
    ...
    public Task<long> SetRange(long offset, T value)
    {
        return Executor.StringSetRangeAsync(KeyName, offset, ToRedisValue(value))
            .ContinueWith(r => long.Parse(r.Result),
                TaskContinuationOptions.ExecuteSynchronously | TaskContinuationOptions.OnlyOnRanToCompletion);
    }
    ...
    /// <returns></returns>
    public static Task<bool> SetMultiple(IList<KeyValuePair<RedisItem<T>, T>> keyValuePairs, When when = When.Always)
    {

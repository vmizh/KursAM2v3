...
public class RedisSortedSet<T> : RedisObject, IAsyncEnumerable<T>
{
    ...
    /// <returns></returns>
    public Task<long> CountByScore(double min = double.NegativeInfinity, double max = double.PositiveInfinity,
    ...
    /// <param name="order"></param>
    /// <returns></returns>
    public Task<IList<(T element, double score)>> RangeWithScores(long start = 0, long stop = -1,
        Order order = Order.Ascending)
    {
        return Executor.SortedSetRangeByRankWithScoresAsync(KeyName, start, stop, order)
            .ContinueWith<IList<(T element, double score)>>(
                r => r.Result.Select(e => (ToElement<T>(e.Element), e.Score)).ToList(),
                TaskContinuationOptions.ExecuteSynchronously | TaskContinuationOptions.OnlyOnRanToCompletion);
    }
    ...
    /// <returns></returns>
    public Task<IList<T>> RangeByValue(T min = default, T max = default, Exclude exclude = Exclude.None,
    ...
    /// <param name="take"></param>
    /// <returns></returns>
    public Task<IList<T>> RangeByScore(double start = double.NegativeInfinity, double stop = double.PositiveInfinity,
    ...
    /// <param name="exclude"></param>
    /// <returns></returns>
    public Task<long> RemoveRangeByScore(double start = double.NegativeInfinity, double stop = double.PositiveInfinity,
        Exclude exclude = Exclude.None)
    {
    ...
    public Task<IList<T>> Sort(Order order = Order.Ascending, SortType sortType = SortType.Numeric, long skip = 0,
        long take = -1, string byKeyNamePattern = null, string[] getKeyNamePattern = null)
    {
    ...
    /// <param name="getKeyNamePattern"></param>
    /// <returns></returns>
    public Task<long> SortAndStore(RedisSortedSet<T> destination, Order order = Order.Ascending,
        SortType sortType = SortType.Numeric, long skip = 0, long take = -1, string byKeyNamePattern = null,
        string[] getKeyNamePattern = null)
    {
        var getKeys = getKeyNamePattern == null ? null : getKeyNamePattern.Select(s => (RedisValue)s).ToArray();
        return Executor.SortAndStoreAsync(destination.KeyName, KeyName, skip, take, order, sortType, byKeyNamePattern,
            getKeys);
    }
    ...
    /// <returns></returns>
    public Task<long> IntersectStore(RedisSortedSet<T> destination, ICollection<RedisSortedSet<T>> otherSets,
        double[] weights = null, Aggregate aggregate = Aggregate.Sum)
    {
        var keys = new[] { (RedisKey)KeyName }.Concat(otherSets.Select(s => (RedisKey)s.KeyName)).ToArray();
        return Executor.SortedSetCombineAndStoreAsync(SetOperation.Intersect, destination.KeyName, keys, weights,
            aggregate);
    }
    ...
    /// <param name="otherSets"></param>
    /// <returns></returns>
    public Task<long> UnionStore(RedisSortedSet<T> destination, ICollection<RedisSortedSet<T>> otherSets,
        double[] weights = null, Aggregate aggregate = Aggregate.Sum)
    {
        if (destination == null) throw new ArgumentException("Destination is required");
        var keys = new[] { (RedisKey)KeyName }.Concat(otherSets.Select(s => (RedisKey)s.KeyName)).ToArray();
        return destination.Executor.SortedSetCombineAndStoreAsync(SetOperation.Union, destination.KeyName, keys,
            weights, aggregate);
    }
    ...
    {
        await foreach (var entry in Executor.SortedSetScanAsync(KeyName)) yield return ToElement<T>(entry.Element);
    }
}
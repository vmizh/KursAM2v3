...
public class RedisSet<T> : RedisObject, IAsyncEnumerable<T>, IEnumerable
{
    ...
    /// <param name="getKeyNamePattern"></param>
    /// <returns></returns>
    public Task<long> SortAndStore(RedisSet<T> destinationSet, Order order = Order.Ascending,
        SortType sortType = SortType.Numeric, long skip = 0, long take = -1, string byKeyNamePattern = null,
        string[] getKeyNamePattern = null)
    {
        var getKeys = getKeyNamePattern == null ? null : getKeyNamePattern.Select(s => (RedisValue)s).ToArray();
        return Executor.SortAndStoreAsync(destinationSet.KeyName, KeyName, skip, take, order, sortType,
            byKeyNamePattern, getKeys);
    }
    ...
    {
        await foreach (var entry in Executor.SetScanAsync(KeyName)) yield return ToElement<T>(entry);
    }

    public IEnumerator GetEnumerator()
    {
        return ToList().Result.GetEnumerator();
    }
}

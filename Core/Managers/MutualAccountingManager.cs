    ...
    public class MutualAccountingManager : DocumentWithRowManager<SD_110ViewModel, TD_110ViewModel>
    {
        ...
        public override SD_110ViewModel Load(decimal dc)
        {
            ...
            using (var ctx = GlobalOptions.GetEntities())
            {
                ...
                doc.UpdateFrom(ent);
                if (doc.SD_111.IsCurrencyConvert)
                    doc.IsOld = false;
                else
                    doc.IsOld = CheckDocumentForOld(dc);
                if (doc.IsOld)
                    doc.DebitorCurrency = GlobalOptions.SystemProfile.MainCurrency;
                doc.State = RowStatus.NotEdited;
                return doc;
            }
        }
                ...
                using (var ctx = GlobalOptions.GetEntities())
                {
                    ...
                    switch (doc.State)
                    {
                        case RowStatus.NewRow:
                            var newDC = ctx.SD_110.Max(_ => _.DOC_CODE) + 1;
                            doc.VZ_NUM = ctx.SD_110.Max(_ => _.VZ_NUM) + 1;
                            doc.DocCode = newDC;
                            ctx.SD_110.Add(new SD_110
                            {
                                ...
                                CurrencyToDC = doc.Entity.CurrencyToDC ?? doc.Entity.CurrencyFromDC
                            });
        ...
        public override TD_110ViewModel AddRow(ObservableCollection<TD_110ViewModel> rows, TD_110ViewModel row, short dolg = 0)
        {
        ...
        public override Tuple<bool, string> IsRowChecked(TD_110ViewModel r)
        {
            ...
            if (string.IsNullOrEmpty(r.VZT_DOC_NUM))
            {
                info.Append("Не установлен номер документа строки.\n");
                ret = false;
            }

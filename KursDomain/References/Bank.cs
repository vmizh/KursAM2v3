...
public class BankViewModel : RSViewModelBase, IEntity<SD_44>
{
    ...
    public BankViewModel(SD_44 entity)
    {
        ...
        if (Entity.SD_114 is { Count: > 0 } 
            || Entity.TD_43 is { Count: > 0 })
            IsUsed = true;
    }
...
public class DataAnnotationsBankViewModel : DataAnnotationForFluentApiBase, IMetadataProvider<BankViewModel>
{
    void IMetadataProvider<BankViewModel>.BuildMetadata(MetadataBuilder<BankViewModel> builder)
    {
        ...
        builder.Property(_ => _.Name).AutoGenerated().DisplayName("Наименование").MaxLength(200);
        builder.Property(_ => _.CORRESP_ACC).AutoGenerated().DisplayName("Корреспондентский счет").MaxLength(30);
        builder.Property(_ => _.ADDRESS).AutoGenerated().DisplayName("Адрес").MaxLength(200);
        builder.Property(_ => _.POST_CODE).AutoGenerated().DisplayName("КПП").MaxLength(10);
    }
}

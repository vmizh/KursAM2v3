...
namespace KursAM2.View.Management
{
    ...
    public partial class ManagementBalansView : ILayout
    {
        ...
        {
            if (!(DataContext is ManagementBalansWindowViewModel ctx) ||
                !(e.NewItem is ManagementBalanceGroupViewModel row)) return;
            ctx.ExtendRowsActual.Clear();
            foreach (var d in ctx.ExtendRows.Where(_ => _.GroupId == row.Id))
                ctx.ExtendRowsActual.Add(d);
        }

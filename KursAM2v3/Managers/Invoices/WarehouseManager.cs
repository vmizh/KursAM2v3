...
namespace KursAM2.Managers.Invoices
{
    public class WarehouseManager
    {
                ...
                using (var transaction = ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            int code = codes.Count > 0 ? codes.Max(_ => _.CODE) : 0;
                        ...
                        foreach (var r in doc.Rows)
                        {
                            if (r.DDT_SPOST_DC != null)
                            {
                                var stores = ctx.TD_24.Where(_ => _.DDT_SPOST_DC == r.DDT_SPOST_DC
                                                                    && _.DDT_SPOST_ROW_CODE == r.DDT_SPOST_ROW_CODE).ToList();
                        ...
                        foreach (var n in doc.Rows.Select(_ => _.Nomenkl.DocCode))
                        {
                            ...
                            if (ops != null && ops.Count > 0)
                                calc.Save(ops);
                            var c = NomenklCalculationManager.GetNomenklStoreRemain(ctx, DateTime.Today, n, (decimal) doc.DD_SKLAD_POL_DC);
                            if (c.Remain < 0)
                            {
                                transaction.Rollback();
        ...
        //TODO заменить параметр WarehouseOrderInSearchViewModel inSearchWindow
                ...
                using (var transaction = ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        foreach (var n in doc.Rows.Select(_ => _.Nomenkl.DocCode))
                        {
                            ...
                            if (ops != null && ops.Count > 0)
                                calc.Save(ops);
                            var c = NomenklCalculationManager.GetNomenklStoreRemain(ctx, DateTime.Today, n, (decimal) doc.DD_SKLAD_OTPR_DC);
                            if (c.Remain < 0)
                            {
                                transaction.Rollback();
                                var nom = MainReferences.GetNomenkl(n);

...
namespace KursAM2.Managers.Invoices
{
    ...
    {
        private ErrorMessageBase errorMessager;
        ...
        {
            return new WarehouseOrderIn(doc.Entity)
            {
                DocCode = -1,
                State = RowStatus.NewRow,
                DD_IN_NUM = -1,
        ...
        {
            var d = GetOrderIn(dc);
            return new WarehouseOrderIn(d.Entity)
            {
                DocCode = -1,
                State = RowStatus.NewRow,
                DD_IN_NUM = -1,
        ...
        public decimal SaveOrderIn(WarehouseOrderIn doc, WarehouseOrderSearchViewModel searchWindow = null)
        {
            ...
            using (var ctx = GlobalOptions.GetEntities())
            {
                using (var transaction = ctx.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            var oldrow = ctx.TD_24.FirstOrDefault(_ => _.DOC_CODE == doc.DocCode && _.CODE == d.Code);
                        ...
                        else
                        {
                            ...
                            var code = ctx.TD_24.Where(_ => _.DOC_CODE == doc.DocCode).Max(_ => _.CODE)+1;
        ...
        public void DeleteOrderIn(WarehouseOrderIn doc, WarehouseOrderSearchViewModel searchWindow = null)
        {
            using (var ctx = GlobalOptions.GetEntities())
            {
                using (var transaction = ctx.Database.BeginTransaction())
            ...
            try
            {
                using (var ctx = GlobalOptions.GetEntities())
                {
                    var data = ctx.SD_24
                        .Include(_ => _.TD_24)
                        //.Include("TD_24.TD_84")
                        //.Include("TD_24.TD_84.SD_84")
                        .FirstOrDefault(_ => _.DOC_CODE == dc);
                    result = new WarehouseOrderOut(data);
                    //var bilingData = (from sd24 in ctx.SD_24
                    //    where sd24.DOC_CODE == dc
                    //    from td24 in ctx.TD_24
                    //    where td24.DOC_CODE == sd24.DOC_CODE
                    //    from td26 in ctx.TD_26
                    //    where td26.DOC_CODE == td24.DDT_SPOST_DC && td26.Code == td24.DDT_SPOST_ROW_CODE
                    //    select td26);
                    //result.biling.Clear();
                    //foreach (var item in bilingData)
                    //{
                    //    result.biling.Add(new InvoiceProviderRow(item));
                    //}
                }
            }
            catch (Exception e)
        ...
        //TODO заменить параметр WarehouseOrderSearchViewModel searchWindow
        public void SaveOrderOut(WarehouseOrderOut doc, WarehouseOrderSearchViewModel searchWindow = null)
        {
        }

        //TODO заменить параметр WarehouseOrderSearchViewModel searchWindow
        public void DeleteOrderOut(WarehouseOrderOut doc, WarehouseOrderSearchViewModel searchWindow = null)
        {
        }

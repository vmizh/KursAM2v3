...
public class ProfitAndLossRepository : IProfitAndLossRepositoryAsync
{
    public async Task<ProfitLossRepositoryResult> CalcTovar(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        string sql;
        if (ProjectIds.Count == 0)
            sql =
                ...
                " , s43.VALUTA_DC KontrCrsDC, s83.NOM_SALE_CRS_DC NomenklCrsDC, TAB.SF_DocDC AS DocumentDC, TAB.SF_Id AS DocumentId " +
                " FROM(SELECT " +
                "   S83.NOM_0MATER_1USLUGA, S84.SF_DATE SF_DATE, s84.DOC_CODE SF_DocDC, s84.Id SF_Id, CAST(S84.SF_IN_NUM AS VARCHAR(50)) + '/' + ISNULL(S84.SF_OUT_NUM, '') SF_NUM " +
            ...
            sql =
                ...
                "      , (SELECT MAX(p2.DATE) FROM NOM_PRICE p2(NOLOCK) WHERE p1.NOM_DC = p2.NOM_DC AND p2.DATE = S24.DD_DATE))) *T24.DDT_KOL_RASHOD AS NUMERIC(18, 4)) AS NomenklSumWOReval " +
                "   FROM TD_84 T84 " +
                $"  INNER JOIN ProjectDocuments pd ON t84.DocId = pd.InvoiceClientId AND pd. pd.ProjectId IN ({makeGuidList(ProjectIds)}) " +
                "  INNER JOIN SD_84 S84 ON S84.DOC_CODE = T84.DOC_CODE " +
                "  INNER JOIN TD_24 T24 ON T24.DDT_SFACT_DC = S84.DOC_CODE " +
                "   AND T24.DDT_SFACT_ROW_CODE = T84.Code AND DDT_KOL_RASHOD > 0 " +
                $" INNER JOIN SD_24 S24  ON S24.DOC_CODE = T24.DOC_CODE AND S24.DD_DATE BETWEEN '{CustomFormat.DateToString(dateStart)}' AND '{CustomFormat.DateToString(dateEnd)}' " +
                "  INNER JOIN SD_83 S83 ON T84.SFT_NEMENKL_DC = S83.DOC_CODE " +
                "  WHERE t84.Id NOT IN (SELECT pre.SFClientRowId FROM ProjectRowExclude pre)) TAB " +
                " INNER JOIN SD_43 s43 ON s43.DOC_CODE = TAB.SF_CLIENT_DC  AND s43.FLAG_BALANS=1 " +
                " INNER JOIN SD_83 s83 ON s83.DOC_CODE = TAB.SFT_NEMENKL_DC " +
                " INNER JOIN SD_50 s50 ON s83.NOM_PRODUCT_DC = s50.DOC_CODE " +
                " LEFT OUTER JOIN EXT_USERS EU ON TAB.CREATOR = EU.USR_NICKNAME " +
        ...
        {
            var data = await ent.Database.SqlQuery<BreakEvenTemp>(sql).ToListAsync();
            ...
            foreach (var e in from d in data
                     let nom =
                         GlobalOptions.ReferencesCache.GetNomenkl(d.NomenklDC) as Nomenkl
            ...
            var nomPrihod = await ent.TD_24
                .Include(_ => _.SD_24)
                .Where(_ =>
                    _.SD_24.DD_DATE >= dateStart && _.SD_24.DD_DATE <= dateEnd &&
                    _.SD_24.DD_TYPE_DC == 2010000005).AsNoTracking().ToListAsync();
            ...
            {
                var nom =
                    GlobalOptions.ReferencesCache.GetNomenkl(d.DDT_NOMENKL_DC) as Nomenkl;
            ...
            var newPrih = new ProfitAndLossesMainRowViewModel
            {
                ...
                CalcType = TypeProfitAndLossCalc.IsProfit,
                LossRUB = 0,
                ProfitRUB = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitRUB),
                ResultRUB = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitRUB),
                LossUSD = 0,
                ProfitUSD = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitUSD),
                ResultUSD = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitUSD),
                LossEUR = 0,
                ProfitEUR = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitEUR),
                ResultEUR = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitEUR),
                LossGBP = 0,
                ProfitGBP = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitGBP),
                ResultGBP = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitGBP),
                LossCHF = 0,
                ProfitCHF = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitCHF),
                ResultCHF = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitCHF),
                LossSEK = 0,
                ProfitSEK = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitSEK),
                ResultSEK = result.Extend.Where(_ => _.GroupId == newPrihodId).Sum(_ => _.ProfitSEK)
            };
            result.Main.Add(newPrih);
            result.MainNach.Add(newPrih);
            newPrih = new ProfitAndLossesMainRowViewModel
            {
                ...
                CalcType = TypeProfitAndLossCalc.IsLoss,
                LossRUB = result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossRUB),
                ProfitRUB = 0,
                ResultRUB = -result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossRUB),
                LossUSD = result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossUSD),
                ProfitUSD = 0,
                ResultUSD = -result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossUSD),
                LossEUR = result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossEUR),
                ProfitEUR = 0,
                ResultEUR = -result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossUSD),
                LossGBP = result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossGBP),
                ProfitGBP = 0,
                ResultGBP = -result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossUSD),
                LossCHF = result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossCHF),
                ProfitCHF = 0,
                ResultCHF = -result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossUSD),
                LossSEK = result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossSEK),
                ProfitSEK = 0,
                ResultSEK = -result.Extend.Where(_ => _.GroupId == spisanieTovara).Sum(_ => _.LossUSD)
            };
            result.Main.Add(newPrih);
            result.MainNach.Add(newPrih);
        }
    ...
    public async Task<ProfitLossRepositoryResult> CalcStartKontragentBalans(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
    ...
    public async Task<ProfitLossRepositoryResult> CalcOutCach(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        using (var ent = GlobalOptions.GetEntities())
        {
            ...
            {
                cashOut = await
                    ent.SD_34.AsNoTracking().Where(_ =>
                        _.NCODE != 100 && _.TABELNUMBER != null && _.DATE_ORD >= dateStart &&
                        _.DATE_ORD <= dateEnd).AsNoTracking().ToListAsync();
            }
            else
            {
                var query = from sd34 in ent.SD_34
                    join pd in ent.ProjectDocuments on sd34.DOC_CODE equals pd.CashOutDC
                    where sd34.TABELNUMBER != null && sd34.DATE_ORD >= dateStart && sd34.DATE_ORD <= dateEnd
                          && ProjectIds.Contains(pd.ProjectId)
                    select sd34;
                cashOut = await query.AsNoTracking().ToListAsync();
            }
    ...
    public async Task<ProfitLossRepositoryResult> CalcStartCash(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using (var ent = GlobalOptions.GetEntities())
        {
            var cashOstatki = await
                ent.TD_22.Include(_ => _.SD_22).AsNoTracking()
                    .Where(_ => _.DATE_START >= dateStart && _.DATE_START <= dateEnd).AsNoTracking()
                    .ToListAsync();
    ...
    public async Task<ProfitLossRepositoryResult> CalcStartBank(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using (var ent = GlobalOptions.GetEntities())
        {
            var bankDates = await
                ent.SD_101.Where(_ => _.VV_START_DATE >= dateStart && _.VV_START_DATE <= dateEnd).AsNoTracking()
                    .ToListAsync();
            if (bankDates.Count == 0) return result;
            ...
            foreach (var d in dates)
            {
                ...
                var dc = (await
                    ent.SD_101.FirstOrDefaultAsync(_ => _.VV_ACC_DC == d.Key && _.VV_START_DATE == d.Value))?.DOC_CODE;
                if (dc == null) continue;
                ...
                foreach (var c in (await ent.UD_101.Where(_ => _.DOC_CODE == dc && _.VVU_REST_TYPE == 0).AsNoTracking()
                             .ToListAsync()).Where(c => c.VVU_VAL_SUMMA > 0))
    ...
    public async Task<ProfitLossRepositoryResult> CalcNomInRounding(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using (var ent = GlobalOptions.GetEntities())
        {
            var dataTemp = await ent.TD_24
                .Include(_ => _.SD_24)
                .Include(_ => _.TD_26)
                .Where(_ =>
                    _.TD_26 != null && _.SD_24.DD_DATE >= dateStart && _.SD_24.DD_DATE <= dateEnd &&
                    // ReSharper disable once EqualExpressionComparison
                    _.TD_26.SFT_ED_CENA.Value != _.TD_26.SFT_ED_CENA.Value).AsNoTracking().ToListAsync();
    ...
    public async Task<ProfitLossRepositoryResult> CalcDilers(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        ...
        if (ProjectIds.Count == 0)
        {
            data = await ctx.TD_24.Include(_ => _.SD_24)
                .Include(_ => _.TD_84)
                .Include(_ => _.TD_84.SD_84)
                .Where(_ => _.SD_24.DD_DATE >= dateStart && _.SD_24.DD_DATE <= dateEnd
                                                         && _.TD_84.SD_84.SF_DILER_DC != null).AsNoTracking()
                .ToListAsync();
        }
        else
        {
            var d = await (from t24 in ctx.TD_24.Include(_ => _.SD_24)
                    .Include(_ => _.TD_84)
                    .Include(_ => _.TD_84.SD_84).Where(_ => _.SD_24.DD_DATE >= dateStart && _.SD_24.DD_DATE <= dateEnd
                        && _.TD_84.SD_84.SF_DILER_DC != null)
                join projDoc in ctx.ProjectDocuments.Where(p => ProjectIds.Contains(p.ProjectId))
                    on t24.TD_84.DocId equals projDoc.InvoiceClientId
                where projDoc.InvoiceClientId != null
                select t24).ToListAsync();
            var exclude = await ctx.ProjectRowExclude
                .Where(_ => ProjectIds.Contains(_.ProjectId) && _.SFClientRowId != null)
                .Select(p => p.SFClientRowId)
                .ToListAsync();
            data = exclude.Count > 0 ? d.Where(_ => !exclude.Contains(_.Id)).ToList() : d;
        }
    ...
    public async Task<ProfitLossRepositoryResult> CalcUslugiDilers(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        using var ctx = GlobalOptions.GetEntities();
        ...
        if (ProjectIds.Count == 0)
        {
            data = await ctx.TD_84
                .Include(_ => _.SD_84)
                .Include(_ => _.SD_83)
                .Where(_ => _.SD_83.NOM_0MATER_1USLUGA == 1 && _.SD_84.SF_DATE >= dateStart &&
                            _.SD_84.SF_DATE <= dateEnd
                            && _.SD_84.SF_DILER_DC != null).AsNoTracking()
                .ToListAsync();
        }
        else
        {
            var d = await (from t84 in ctx.TD_84
                    .Include(_ => _.SD_84)
                    .Include(_ => _.SD_83)
                    .Where(_ => _.SD_83.NOM_0MATER_1USLUGA == 1 && _.SD_84.SF_DATE >= dateStart &&
                                _.SD_84.SF_DATE <= dateEnd
                ...
                join projDoc in ctx.ProjectDocuments.Where(p => ProjectIds.Contains(p.ProjectId))
                    on t84.DocId equals projDoc.InvoiceClientId
                where projDoc.InvoiceClientId != null
                select t84).AsNoTracking().ToListAsync();
        }
        ...
        foreach (var d in data)
        {
            ...
            var nom =
                GlobalOptions.ReferencesCache.GetNomenkl(d.SD_83.DOC_CODE) as Nomenkl;
            var newOp = new ProfitAndLossesExtendRowViewModel
            {
                GroupId = Guid.Parse("{BA628F86-6AE4-4CF3-832B-C6F7388DD01B}"),
                Name = nom.Name,
    ...
    public async Task<ProfitLossRepositoryResult> CalcVozvrat(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using var ctx = GlobalOptions.GetEntities();
        var vozvratTovara = await ctx.TD_24.Include(_ => _.SD_24)
            .Where(_ => _.DDT_SPOST_DC == null
                        && _.SD_24.DD_DATE >= dateStart && _.SD_24.DD_DATE <= dateEnd
                        && _.SD_24.DD_VOZVRAT == 1).AsNoTracking()
            .ToListAsync();
        ...
        foreach (var d in vozvratTovara)
        {
            ...
            var p = await GetNomenklPriceAsync(nom.DocCode, d.SD_24.DD_DATE, ctx);
            SetCurrenciesValue(newOp1, ((IDocCode)nom.Currency).DocCode, p.Price * d.DDT_KOL_PRIHOD, 0);

            result.Extend.Add(newOp1);
            result.ExtendNach.Add(newOp1);
        }

        return result;
    }

    public async Task<ProfitLossRepositoryResult> CalcNomenklReturn(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using var ctx = GlobalOptions.GetEntities();

        var returnClient = await ctx.NomenklReturnOfClientRow
            .Include(_ => _.NomenklReturnOfClient)
            .Where(_ => _.NomenklReturnOfClient.DocDate >= dateStart &&
                        _.NomenklReturnOfClient.DocDate <= dateEnd).AsNoTracking().ToListAsync();
        foreach (var d in returnClient)
        {
            var kontr =
                GlobalOptions.ReferencesCache.GetKontragent(d.NomenklReturnOfClient.KontragentDC) as Kontragent;
            var nom =
                GlobalOptions.ReferencesCache.GetNomenkl(d.NomenklDC) as Nomenkl;
            var newOp = new ProfitAndLossesExtendRowViewModel
            {
            ...
            var p = await GetNomenklPriceAsync(nom.DocCode, d.NomenklReturnOfClient.DocDate, ctx);
        ...
        var returnProvider = await ctx.NomenklReturnToProviderRow.Include(_ => _.NomenklReturnToProvider)
            .Where(_ => _.NomenklReturnToProvider.DocDate >= dateStart &&
                        _.NomenklReturnToProvider.DocDate <= dateEnd).AsNoTracking().ToListAsync();
        foreach (var d in returnProvider)
        {
            ...
            result.Extend.Add(newOp2);
            result.ExtendNach.Add(newOp2);
            ...
            var p = await GetNomenklPriceAsync(nom.DocCode, d.NomenklReturnToProvider.DocDate, ctx);
            SetCurrenciesValue(newOp3, ((IDocCode)nom.Currency).DocCode, 0, p.Price * d.Quantity);

            result.Extend.Add(newOp3);
            result.ExtendNach.Add(newOp3);
        }

        return result;
    }

    public async Task<ProfitLossRepositoryResult> CalcSpisanie(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using var ctx = GlobalOptions.GetEntities();

        var spisano = await ctx.TD_24.Include(_ => _.SD_24)
    ...
    public async Task<ProfitLossRepositoryResult> CalcTovarTransfer(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using var ctx = GlobalOptions.GetEntities();

        var data = await ctx.NomenklTransferRow
            .Include(_ => _.NomenklTransfer)
            .Include(_ => _.SD_83)
            .Include(_ => _.SD_831)
            .Where(_ =>
                _.NomenklTransfer.Date >= dateStart && _.NomenklTransfer.Date <= dateEnd &&
                _.IsAccepted).AsNoTracking().ToListAsync();
        foreach (var row in data)
        {
            ...
            var newOp = new ProfitAndLossesExtendRowViewModel
            {
                ...
                Nomenkl =
                    GlobalOptions.ReferencesCache.GetNomenkl(row.SD_83.DOC_CODE) as Nomenkl
            };
            SetCurrenciesValue(newOp, crs.DocCode, row.PriceIn * row.Quantity, 0m);
            var nomPrice = await ctx.NOM_PRICE.FirstOrDefaultAsync(_ => _.NOM_DC == row.SD_83.DOC_CODE &&
                                                                        _.DATE ==
                                                                        ctx.NOM_PRICE.Where(c =>
                                                                                c.NOM_DC == row.SD_83.DOC_CODE &&
                                                                                c.DATE <= row.NomenklTransfer.Date)
                                                                            .Max(d => d.DATE));
            var price = nomPrice?.PRICE_WO_NAKLAD ?? 0;
            var newOp1 = new ProfitAndLossesExtendRowViewModel
            {
                GroupId = Guid.Parse("{564DB69C-6DAD-4B16-8BF5-118F5AF2D07F}"),
        ...
        //TODO Обязательно проверить на Main записи, по умолчанию их нет, надо занести
        var idNach = Guid.Parse("{564DB69C-6DAD-4B16-8BF5-118F5AF2D07F}");
        var n = result.Main.FirstOrDefault(_ => _.Id == idNach);
        var n1 = result.MainNach.FirstOrDefault(_ => _.Id == idNach);

        // ReSharper disable once PossibleNullReferenceException
        n.LossRUB = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossRUB);
        n.ProfitRUB = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitRUB);
        n.ResultRUB = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitRUB - _.LossRUB);
        n.LossUSD = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossUSD);
        n.ProfitUSD = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitUSD);
        n.ResultUSD = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitUSD - _.LossUSD);
        n.LossEUR = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossEUR);
        n.ProfitEUR = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitEUR);
        n.ResultEUR = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitEUR - _.LossEUR);
        n.LossGBP = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossGBP);
        n.ProfitGBP = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitGBP);
        n.ResultGBP = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitGBP - _.LossGBP);
        n.LossCHF = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossCHF);
        n.ProfitCHF = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitCHF);
        n.ResultCHF = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitCHF - _.LossCHF);
        n.LossSEK = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossSEK);
        n.ProfitSEK = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitSEK);
        n.ResultSEK = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitSEK - _.LossSEK);

        // ReSharper disable once PossibleNullReferenceException
        n1.LossRUB = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossRUB);
        n1.ProfitRUB = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitRUB);
        n1.ResultRUB = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitRUB - _.LossRUB);
        n1.LossUSD = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossUSD);
        n1.ProfitUSD = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitUSD);
        n1.ResultUSD = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitUSD - _.LossUSD);
        n1.LossEUR = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossEUR);
        n1.ProfitEUR = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitEUR);
        n1.ResultEUR = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitEUR - _.LossEUR);
        n1.LossGBP = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossGBP);
        n1.ProfitGBP = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitGBP);
        n1.ResultGBP = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitGBP - _.LossGBP);
        n1.LossCHF = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossCHF);
        n1.ProfitCHF = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitCHF);
        n1.ResultCHF = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitCHF - _.LossCHF);
        n1.LossSEK = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.LossSEK);
        n1.ProfitSEK = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitSEK);
        n1.ResultSEK = result.Extend.Where(_ => _.GroupId == idNach).Sum(_ => _.ProfitSEK - _.LossSEK);


        return result;
    }

    public async Task<ProfitLossRepositoryResult> CalcUslugi(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        using var ctx = GlobalOptions.GetEntities();
        ...
        {
            var dataIn =
                new List<ProfitAndLossesWindowViewModel2.NakladTemp>();
            var dataOut =
                new List<ProfitAndLossesWindowViewModel2.NakladTemp>();
            if (ProjectIds.Count == 0)
            {
                dataIn = await (from sd84 in ent.SD_84.Include(_ => _.SD_40)
                ...
                dataOut = await (from sd26 in ent.SD_26.Include(_ => _.SD_40)
                    // ReSharper disable once AccessToDisposedClosure
            ...
            {
                var sfClientInclude = await ent.ProjectDocuments
                    .Where(_ => ProjectIds.Contains(_.ProjectId) && _.InvoiceClientId != null)
                    .Select(x => x.InvoiceClientId).ToListAsync();
                var excludeClient = await ent.ProjectRowExclude
                    .Where(_ => ProjectIds.Contains(_.ProjectId) && _.SFClientRowId != null)
                    .Select(x => x.SFClientRowId).ToListAsync();
                dataIn = await (from sd84 in ent.SD_84.Include(_ => _.SD_40).Where(_ => sfClientInclude.Contains(_.Id))
                    // ReSharper disable once AccessToDisposedClosure
                    from td84 in ent.TD_84.Where(_ => !excludeClient.Contains(_.Id))
                    // ReSharper disable once AccessToDisposedClosure
                    from sd83 in ent.SD_83
                    //from sd40 in ent.SD_40
                    // ReSharper disable once AccessToDisposedClosure
                ...
                foreach (var dd in dataIn) dd.Quantity = Convert.ToDecimal(dd.DoubleQuantity);
                var sfProviderInclude = await ent.ProjectDocuments
                    .Where(_ => ProjectIds.Contains(_.ProjectId) && _.InvoiceProviderId != null)
                    .Select(x => x.InvoiceClientId).ToListAsync();
                var excludeProvider = await ent.ProjectRowExclude
                    .Where(_ => ProjectIds.Contains(_.ProjectId) && _.SFProviderRowId != null)
                    .Select(x => x.SFProviderRowId).ToListAsync();
                dataOut = await (from sd26 in ent.SD_26.Include(_ => _.SD_40)
                        .Where(_ => sfProviderInclude.Contains(_.Id))
                    // ReSharper disable once AccessToDisposedClosure
                    from td26 in ent.TD_26.Where(_ => !excludeProvider.Contains(_.Id))
                    // ReSharper disable once AccessToDisposedClosure
                    from sd83 in ent.SD_83
                    //from sd40 in ent.SD_40
                    // ReSharper disable once AccessToDisposedClosure 
            ...
            foreach (var d in dataIn)
            {
                ...
                var e = new ProfitAndLossesExtendRowViewModel
                {
                    ...
                    // ReSharper disable once PossibleInvalidOperationException
                    SDRSchet = sdrSchet,
                    SDRState = sdrSchet?.SDRState as SDRState
                };
            ...
            //TODO Проверить на наличие, по умолчанию их там нет.
            var rm1 = result.Main.Where(_ => _.ParentId == Guid.Parse("{2FA1DD9F-6842-4209-B0CC-DDEF3B920496}"))
                .ToList();
            var rmn1 = result.MainNach.Where(_ => _.ParentId == Guid.Parse("{2FA1DD9F-6842-4209-B0CC-DDEF3B920496}"))
                .ToList();
            foreach (var m in rm1) result.Main.Remove(m);
            foreach (var mn in rmn1) result.MainNach.Remove(mn);
            foreach (
                var n in dictCOIns.Select(d =>
                    new
                        ProfitAndLossesMainRowViewModel
                        {
                            ...
                            LossRUB = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.LossRUB),
                            ProfitRUB = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ProfitRUB),
                            ResultRUB = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ResultRUB),
                            LossUSD = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.LossUSD),
                            ProfitUSD = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ProfitUSD),
                            ResultUSD = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ResultUSD),
                            LossEUR = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.LossEUR),
                            ProfitEUR = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ProfitEUR),
                            ResultEUR = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ResultEUR),
                            LossGBP = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.LossGBP),
                            ProfitGBP = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ProfitGBP),
                            ResultGBP = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ResultGBP),
                            LossCHF = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.LossCHF),
                            ProfitCHF = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ProfitCHF),
                            ResultCHF = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ResultCHF),
                            LossSEK = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.LossSEK),
                            ProfitSEK = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ProfitSEK),
                            ResultSEK = result.Extend.Where(_ => _.GroupId == d.Value).Sum(_ => _.ResultSEK)
                        }
                ))
            {
                result.Main.Add(n);
            ...
            //TODO Прочверить на наличие, по умолчангию ничего нет.
            var rm = result.Main.Where(_ => _.ParentId == Guid.Parse("{E47EF726-3BEA-4B18-9773-E564D624FDF6}"))
                .ToList();
            var rmn = result.MainNach.Where(_ => _.ParentId == Guid.Parse("{E47EF726-3BEA-4B18-9773-E564D624FDF6}"))
                .ToList();
            foreach (var m in rm) result.Main.Remove(m);
            foreach (var mn in rmn) result.MainNach.Remove(mn);
            foreach (
    ...
    public async Task<ProfitLossRepositoryResult> CalcFinance(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using var ent = GlobalOptions.GetEntities();

        var data = await (from sd110 in ent.SD_110
            // ReSharper disable once AccessToDisposedClosure
    ...
    public async Task<ProfitLossRepositoryResult> CalcOutBalans(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using var ent = GlobalOptions.GetEntities();

        var dataTovarPoluchen = await (from td24 in ent.TD_24
        ...
        var dataTovarOtgr = await (from td24 in ent.TD_24
            from sd24 in ent.SD_24
            from sd43 in ent.SD_43
            from td84 in ent.TD_84
            from sd83 in ent.SD_83
            ...
            where td24.DOC_CODE == sd24.DOC_CODE
                  ...
                  && sd24.DD_KONTR_OTPR_DC != GlobalOptions.SystemProfile.OwnerKontragent.DocCode
                  && nomPrice.NOM_DC == td24.DDT_NOMENKL_DC && nomPrice.DATE == ent.NOM_PRICE.Where(_ =>
                          _.NOM_DC ==
                          td24.DDT_NOMENKL_DC &&
                          _.DATE <= sd24.DD_DATE)
                      .Max(_ => _.DATE)
        ...
        var dataBankIn = await (from sd101 in ent.SD_101
        ...
        var dataBankOut = await (from sd101 in ent.SD_101
            from td101 in ent.TD_101
            from sd43 in ent.SD_43
        ...
        var dataCashIn = await (from sd33 in ent.SD_33
            // ReSharper disable once AccessToDisposedClosure
            from sd43 in ent.SD_43
        ...
        var dataCashOut = await (from sd34 in ent.SD_34
            // ReSharper disable once AccessToDisposedClosure
            from sd43 in ent.SD_43
    ...
    public async Task<ProfitLossRepositoryResult> SpisanieTovar(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using var ctx = GlobalOptions.GetEntities();

        var data = await ctx.AktSpisaniya_row.Include(_ => _.AktSpisaniyaNomenkl_Title)
            .Where(_ => _.AktSpisaniyaNomenkl_Title.Date_Doc >= dateStart &&
                        _.AktSpisaniyaNomenkl_Title.Date_Doc <= dateEnd).AsNoTracking().ToListAsync();
        foreach (var d in data)
        {
            var nom = GlobalOptions.ReferencesCache.GetNomenkl(d.Nomenkl_DC) as Nomenkl;
            var newOp = new ProfitAndLossesExtendRowViewModel
            {
                ...
                Quantity = d.Quantity,
                Price =
                    GetNomenklPriceAsync(d.Nomenkl_DC, d.AktSpisaniyaNomenkl_Title.Date_Doc, ctx).Result?.Price ?? 0,
    ...
    public async Task<ProfitLossRepositoryResult> CalcCurrencyChange(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using var ent = GlobalOptions.GetEntities();

        var cashChanged = await ent.SD_251.Include(_ => _.SD_22)
            .Include(_ => _.SD_301)
            .Include(_ => _.SD_3011)
            .Where(_ => _.CH_DATE >= dateStart && _.CH_DATE <= dateEnd).AsNoTracking()
            .ToListAsync();
        ...
        var bankChanged = await ent.TD_101.Include(_ => _.SD_101)
    ...
    public async Task<ProfitLossRepositoryResult> CalcZarplata(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using var ent = GlobalOptions.GetEntities();

        var data = await (from sd34 in ent.SD_34.AsNoTracking()
            from sd2 in ent.SD_2.AsNoTracking()
        ...
        var data2 = await (from td101 in ent.TD_101.Include(_ => _.SD_101).AsNoTracking()
            from sd2 in ent.SD_2.AsNoTracking()
    ...
    public async Task<ProfitLossRepositoryResult> CalcNomenklCurrencyChanged(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        using var ctx = GlobalOptions.GetEntities();
        List<TD_26_CurrencyConvert> nomChanged;
        if (ProjectIds.Count == 0)
        {
            nomChanged = await ctx.TD_26_CurrencyConvert
                .Include(_ => _.TD_26)
                .Include(_ => _.TD_26.SD_26).AsNoTracking()
                .Where(_ => _.Date >= dateStart && _.Date <= dateEnd && _.DOC_CODE != null).AsNoTracking()
                .ToListAsync();
        }
        else
        {
            var sfInclude = await ctx.ProjectDocuments
                .Where(_ => ProjectIds.Contains(_.ProjectId) && _.InvoiceProviderId != null)
                .Select(x => x.InvoiceProviderId).ToListAsync();
            var exclude = await ctx.ProjectRowExclude
                .Where(_ => ProjectIds.Contains(_.ProjectId) && _.NomCurrencyConvertRowId != null)
                .Select(x => x.SFClientRowId).ToListAsync();

            nomChanged = await ctx.TD_26_CurrencyConvert.Where(_ => !exclude.Contains(_.Id))
                .Include(_ => _.TD_26)
                .Include(_ => _.TD_26.SD_26).AsNoTracking()
                .Where(_ => _.Date >= dateStart && _.Date <= dateEnd && _.DOC_CODE != null &&
                            sfInclude.Contains(_.TD_26.DocId)).AsNoTracking()
                .ToListAsync();
        }

        foreach (var n in nomChanged)
        {
            var nom = GlobalOptions.ReferencesCache.GetNomenkl(n.NomenklId) as Nomenkl;
    ...
    public async Task<ProfitLossRepositoryResult> CalcAccruedAmmount(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        using var ctx = GlobalOptions.GetEntities();
        List<AccruedAmountForClient> outBalansClient = [];
        List<AccuredAmountOfSupplierRow> outBalansSupplier = [];
        if (ProjectIds.Count == 0)
        {
            outBalansClient = await ctx.AccruedAmountForClient
                .Include(_ => _.AccuredAmountForClientRow)
                .Where(_ => _.DocDate >= dateStart && _.DocDate <= dateEnd).AsNoTracking().ToListAsync();

            outBalansSupplier = await ctx.AccuredAmountOfSupplierRow
                ...
                .Where(_ => _.AccruedAmountOfSupplier.DocDate >= dateStart &&
                            _.AccruedAmountOfSupplier.DocDate <= dateEnd).AsNoTracking().ToListAsync();
        }
        else
        {
            var rowClients = await ctx.ProjectDocuments
                .Where(_ => ProjectIds.Contains(_.ProjectId) && _.AccruedClientRowId != null)
                .Select(x => x.AccruedClientRowId).ToListAsync();
            var rowSuppliers = await ctx.ProjectDocuments
                .Where(_ => ProjectIds.Contains(_.ProjectId) && _.AccuredAmountOfSupplierRow != null)
                .Select(x => x.AccruedSupplierRowId).ToListAsync();
            var d = await ctx.AccruedAmountForClient
                .Include(_ => _.AccuredAmountForClientRow)
                .Where(_ => _.DocDate >= dateStart && _.DocDate <= dateEnd).AsNoTracking().ToListAsync();
            foreach (var r in d)
                outBalansClient.AddRange(from r1 in r.AccuredAmountForClientRow
                    where rowClients.Contains(r1.Id)
                    select r);
            ...
            outBalansSupplier = await ctx.AccuredAmountOfSupplierRow
                ...
                .Include(accuredAmountOfSupplierRow => accuredAmountOfSupplierRow.TD_101)
                .Where(_ => _.AccruedAmountOfSupplier.DocDate >= dateStart &&
                            _.AccruedAmountOfSupplier.DocDate <= dateEnd && rowSuppliers.Contains(_.Id)).AsNoTracking()
                .ToListAsync();
        }
        ...
        foreach (var d in outBalansSupplier)
        {
            ...
            var newOp1 = new ProfitAndLossesExtendRowViewModel
            {
                ...
                Price = d.Summa,
                KontragentName =
                    ((IName)GlobalOptions.ReferencesCache.GetKontragent(d.AccruedAmountOfSupplier.KontrDC)).Name,
    ...
    public async Task<ProfitLossRepositoryResult> CalcStockHolders(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using var ctx = GlobalOptions.GetEntities();

        var data = await ctx.SD_34.Include(_ => _.StockHolders)
            .AsNoTracking().Where(_ => _.DATE_ORD >= dateStart && _.DATE_ORD <= dateEnd
                                                               && _.StockHolderId != null).AsNoTracking().ToListAsync();
        foreach (var d in data)
        {
            var newOp = new ProfitAndLossesExtendRowViewModel
            {
    ...
    public async Task<ProfitLossRepositoryResult> CalcTransferOutBalans(DateTime dateStart, DateTime dateEnd)
    {
        var result = new ProfitLossRepositoryResult();
        if (ProjectIds.Count > 0) return result;
        using var ctx = GlobalOptions.GetEntities();

        var data = await ctx.TransferOutBalansRows.Include(_ => _.TransferOutBalans)
    ...
    private readonly List<CURRENCY_RATES_CB> MyRates = [];
    private Guid spisanieTovara;
    private readonly List<Guid> ProjectIds = [];
    ...
    public ProfitAndLossRepository()
    {
        using (var ctx = GlobalOptions.GetEntities())
        {
            MyRates = ctx.CURRENCY_RATES_CB.ToList();
        }
    }

    public ProfitAndLossRepository(Project project) : this()
    {
        ProjectIds = getAllPrjectsId(project);
    }
    ...
    public async Task<Prices> GetNomenklPriceAsync(decimal nomdc, DateTime date, ALFAMEDIAEntities context)
    {
        ...
        var data1 = await context.Database.SqlQuery<Prices>(sql).ToListAsync();
        if (data1.Count == 0)
            return new Prices
            {
                Price = 0,
    ...
    private void SetCurrenciesValue(IProfitCurrencyList item, decimal? crsDC, decimal? profit, decimal? loss)
    {
    ...
    private decimal GetRate(List<CURRENCY_RATES_CB> rates, decimal firstDC, decimal secondDC, DateTime date)
    {
    ...
    private List<Guid> getAllPrjectsId(Project project)
    {
        var res = new List<Guid>();
        using var ctx = GlobalOptions.GetEntities();
        var projRepository = new ProjectRepository(ctx);
        var projIds = projRepository.GetChilds(project.Id);
        return ctx.Projects.Where(_ => (_.IsExcludeFromProfitAndLoss ?? false) == false && projIds.Contains(_.Id))
            .Select(x => x.Id).ToList();
    }

    private string makeGuidList(List<Guid> list)
    {
        var guidList = new StringBuilder();
        for (var i = 0; i < list.Count - 1; i++)
            guidList.Append(i < ProjectIds.Count - 1 ? $"'{list[i]}'," : $"'{list[i]}'");

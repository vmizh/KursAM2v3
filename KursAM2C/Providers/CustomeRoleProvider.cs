...
namespace KursAM2C.Providers
{
    public class CustomeRoleProvider : RoleProvider
    {
        ...
        public override bool IsUserInRole(string username, string roleName)
        {
            ...
            {
                var user = ctx.EXT_USERS.FirstOrDefault(_ => string.Equals(_.USR_NICKNAME, username, StringComparison.CurrentCultureIgnoreCase));
                if (user != null)
                    outputResult = user.EXT_GROUPS?.FirstOrDefault(_ => string.Equals(_.GR_NAME, roleName, StringComparison.CurrentCultureIgnoreCase)) !=
                                   null;
            }

...
#if NETCORE
        var hasIdInterfaces =
 typeof(TEntity).GetTypeInfo().ImplementedInterfaces.Where(t => t.GetTypeInfo().IsGenericType 
            && t.GetTypeInfo().GetGenericTypeDefinition() == typeof(IHasId<>)).ToArray();
#else
...
public static class IdUtils
{
    ...
    {
        return entity.GetType().GetIdProperty().GetGetMethod(true).Invoke(entity, TypeConstants.EmptyObjectArray);
    }
    ...
    {
        if (idValue.Length < 4) idValue = idValue.PadLeft(4, '0');
        ...
        var path =
            $"{rootDir}{PclExport.Instance.DirSep}{dir1}{PclExport.Instance.DirSep}{dir2}{PclExport.Instance.DirSep}{idValue}";

...
public class CsvReader
{
    ...
    public static string EatValue(string value, ref int i)
    {
        ...
        while (++i < valueLength) //Is Value
        {
            ...
            if (valueChar == itemSeperator || valueChar == specEndChar) break;
        }
...
public class CsvReader<T>
{
    ...
    internal static void Reset()
    {
        ...
        PropertySettersMap =
            new Dictionary<string, SetMemberDelegate<T>>(PclExport.Instance.InvariantComparerIgnoreCase);

        PropertyConverters = new List<ParseStringDelegate>();
        PropertyConvertersMap =
            new Dictionary<string, ParseStringDelegate>(PclExport.Instance.InvariantComparerIgnoreCase);
    ...
    public static List<T> GetRows(IEnumerable<string> records)
    {
        ...
        if (typeof(T).IsValueType || typeof(T) == typeof(string)) return GetSingleRow(records, typeof(T));

        foreach (var record in records)
        {
            var to = typeof(T).CreateInstance<T>();
            foreach (var propertySetter in PropertySetters) propertySetter(to, record);
            rows.Add(to);
        }
    ...
    public static List<T> Read(List<string> rows)
    {
        ...
        List<string> headers = null;
        if (!CsvConfig<T>.OmitHeaders || Headers.Count == 0) headers = CsvReader.ParseFields(rows[0], s => s.Trim());

...
public static class DateTimeSerializer
{
    ...
    private static readonly Regex UtcOffsetInfoRegex =
        new Regex("([+-](?:2[0-3]|[0-1][0-9]):[0-5][0-9])", RegexOptions.Compiled);

    private static readonly char[] TimeZoneChars = { '+', '-' };
    ...
    /// <summary>
    ///     If AlwaysUseUtc is set to true then convert all DateTime to UTC. If PreserveUtc is set to true then UTC dates will
    ///     not convert to local
    /// </summary>
    ...
    public static DateTime ParseShortestXsdDateTime(string dateTimeStr)
    {
        ...
        if (config.SystemJsonCompatible)
        {
            ...
            {
                var value =
 System.Text.Json.JsonSerializer.Deserialize<DateTime>('"' + dateTimeStr + '"', TextConfig.SystemJsonOptions);
                return value;
            }
            catch (Exception e)
            {
        ...
        try
        {
            ...
            if (dateTimeStr.Length == XsdDateTimeFormatSeconds.Length)
                return DateTime
                    .ParseExact(dateTimeStr, XsdDateTimeFormatSeconds, null, DateTimeStyles.AdjustToUniversal)
                    .Prepare(true);
            ...
            if (dateTimeStr.Length == CondensedDateTimeFormat.Length &&
                dateTimeStr.IndexOfAny(DateTimeSeparators) == -1)
    ...
    /// <summary>
    ///     Repairs an out-of-spec XML date/time string which incorrectly uses a space instead of a 'T' to separate the date
    ///     from the time.
    ///     These string are occasionally generated by SQLite and can cause errors in OrmLite when reading these columns from
    ///     the DB.
    /// </summary>
    ...
    {
        if (dateTimeStr.Length > XsdTimeSeparatorIndex && dateTimeStr[XsdTimeSeparatorIndex] == ' ' &&
    ...
    public static DateTime? ParseManual(string dateTimeStr, DateTimeKind dateKind)
    {
        ...
        if (parts.Length == 2)
        {
            ...
            else
            {
                ...
                if (timeStringParts.Length == 2) offsetMultiplier = 1;
            }
            ...
            var dateTime = new DateTime(int.Parse(dateParts[0]), int.Parse(dateParts[1]), int.Parse(dateParts[2]), hh,
                min,
                ss, ms, dateKind);
            if (subMs != 0) dateTime = dateTime.AddMilliseconds(subMs);
    ...
    public static DateTimeOffset ParseDateTimeOffset(string dateTimeOffsetStr)
    {
        ...
        if (config.SystemJsonCompatible)
        {
            ...
            {
                var value =
 System.Text.Json.JsonSerializer.Deserialize<DateTimeOffset>('"' + dateTimeOffsetStr + '"', TextConfig.SystemJsonOptions);
                return value;
            }
            catch (Exception e)
            {
    ...
    {
        return dateTimeStr.StartsWith("P", StringComparison.Ordinal) ||
               dateTimeStr.StartsWith("-P", StringComparison.Ordinal)
    ...
    {
        return string.IsNullOrEmpty(dateTimeStr) ? null : new TimeSpan?(ParseXsdTimeSpan(dateTimeStr));
    }
                ...
                return dateTime.Kind == DateTimeKind.Local
                    ...
                    : dateTime.Kind == DateTimeKind.Unspecified
                        ...
                        : dateTime.ToStableUniversalTime()
                            .ToString(XsdDateTimeFormatSeconds, CultureInfo.InvariantCulture);
    ...
    public static DateTimeOffset ParseWcfJsonDateOffset(string wcfJsonDate)
    {
        ...
        if (wcfJsonDate[0] == '\\') wcfJsonDate = wcfJsonDate.Substring(1);

        var suffixPos = wcfJsonDate.IndexOf(WcfJsonSuffix);
        var timeString = suffixPos < 0
            ? wcfJsonDate
            : wcfJsonDate.Substring(WcfJsonPrefix.Length, suffixPos - WcfJsonPrefix.Length);
    ...
    {
        if (wcfJsonDate[0] == JsonUtils.EscapeChar) wcfJsonDate = wcfJsonDate.Substring(1);

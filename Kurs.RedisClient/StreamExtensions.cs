...
public static class StreamExtensions
{
    ...
    public static byte[] ReadFully(this Stream input, byte[] buffer)
    {
        ...
        if (tempStream.Length == tempStream.GetBuffer().Length) return tempStream.GetBuffer();
    ...
    {
        return ReadFullyAsync(input, DefaultBufferSize, token);
    }
    ...
    ///     array, using the given buffer size.
    /// </summary>
    public static async Task<byte[]> ReadFullyAsync(this Stream input, int bufferSize,
    ...
    {
        return ReadFullyAsMemory(input, DefaultBufferSize);
    }
    ...
    ///     Reads the given stream up to the end, returning the MemoryStream Buffer as ReadOnlyMemory&lt;byte&gt;.
    /// </summary>
    public static Task<ReadOnlyMemory<byte>> ReadFullyAsMemoryAsync(this Stream input,
        CancellationToken token = default)
    {
        return ReadFullyAsMemoryAsync(input, DefaultBufferSize, token);
    }

    /// <summary>
    ///     Reads the given stream up to the end, returning the MemoryStream Buffer as ReadOnlyMemory&lt;byte&gt;.
    /// </summary>
    public static async Task<ReadOnlyMemory<byte>> ReadFullyAsMemoryAsync(this Stream input, int bufferSize,
        CancellationToken token = default)
    {
        var buffer = BufferPool.GetBuffer(bufferSize);
        try
    ...
    public static async Task<ReadOnlyMemory<byte>> ReadFullyAsMemoryAsync(this Stream input, byte[] buffer,
        CancellationToken token = default)
    {
        if (buffer == null)
            throw new ArgumentNullException(nameof(buffer));
    ...
    ///     the buffer is ignored, so the buffer needn't be cleared beforehand.
    /// </summary>
    public static async Task<long> CopyToAsync(this Stream input, Stream output, byte[] buffer,
        CancellationToken token = default)
    {
        if (buffer == null)
            throw new ArgumentNullException(nameof(buffer));
    ...
    {
        return MemoryProvider.Instance.WriteAsync(stream, bytes, token);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Task CopyToAsync(this Stream input, Stream output, CancellationToken token = default)
    {
        return input.CopyToAsync(output, AsyncBufferSize, token);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Task WriteAsync(this Stream stream, string text, CancellationToken token = default)
    {
        return MemoryProvider.Instance.WriteAsync(stream, text.AsMemory(), token);
    }
    ...
    {
        return ToMd5Bytes(stream).ToHex();
    }

    public static string ToMd5Hash(this byte[] bytes)
    {
        return MD5.Create().ComputeHash(bytes).ToHex();
    }
    ...
    public static string ReadToEnd(this MemoryStream ms, Encoding encoding)
    {
        ...
        Tracer.Instance.WriteWarning(
            "MemoryStream wasn't created with a publiclyVisible:true byte[] buffer, falling back to slow impl");
    ...
    public static ReadOnlyMemory<byte> GetBufferAsMemory(this MemoryStream ms)
    {
        ...
        Tracer.Instance.WriteWarning(
            "MemoryStream in GetBufferAsSpan() wasn't created with a publiclyVisible:true byte[] buffer, falling back to slow impl");
    ...
    public static byte[] GetBufferAsBytes(this MemoryStream ms)
    {
        ...
        Tracer.Instance.WriteWarning(
            "MemoryStream in GetBufferAsBytes() wasn't created with a publiclyVisible:true byte[] buffer, falling back to slow impl");
        return ms.ToArray();
    }

    public static Task<string> ReadToEndAsync(this MemoryStream ms)
    {
        return ReadToEndAsync(ms, JsConfig.UTF8Encoding);
    }
    ...
    public static Task<string> ReadToEndAsync(this MemoryStream ms, Encoding encoding)
    {
        ...
        Tracer.Instance.WriteWarning(
            "MemoryStream in ReadToEndAsync() wasn't created with a publiclyVisible:true byte[] buffer, falling back to slow impl");
    ...
    {
        return ReadToEnd(stream, JsConfig.UTF8Encoding);
    }
    ...
    {
        return ReadToEndAsync(stream, JsConfig.UTF8Encoding);
    }
    ...
    {
        return WriteToAsync(stream, output, JsConfig.UTF8Encoding, token);
    }
...
#endif
        Tracer.Instance.WriteWarning(
            "MemoryStream in WriteToAsync() wasn't created with a publiclyVisible:true byte[] bufffer, falling back to slow impl");
    ...
    ///     Writes partial range as specified by start-end, from fromStream to toStream.
    /// </summary>
    public static async Task WritePartialToAsync(this Stream fromStream, Stream toStream, long start, long end,
        CancellationToken token = default)
    {
        if (!fromStream.CanSeek)
            throw new InvalidOperationException(
